...
class CalciteMaterializer extends TrainDBPrepareImpl.TrainDBPreparingStmt {
  ...
  void populate(TrainDBPrepareImpl.TrainDBMaterialization materialization) {
    ...
    materialization.queryRel_ = trimUnusedFields(root).rel;
    ...
    List<String> tableName = materialization.materializedTable_.path();
    RelOptTable table = requireNonNull(
        this.catalogReader.getTable(tableName),
        () -> "table " + tableName + " is not found");
    materialization.tableRel_ = sqlToRelConverter2.toRel(table, ImmutableList.of());
  }
    ...
    for (Callback x : useStar(schema, queryRel)) {
      ...
      if (CalciteSystemProperty.DEBUG.value()) {
        System.out.println("Materialization "
            + materialization.materializedTable_ + " matched star table "
