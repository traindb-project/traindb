...
public class TrainDBPlanner implements Planner, ViewExpander {
  ...
  public TrainDBPlanner(FrameworkConfig config) {
  ...
  private void ready() {
    ...
    // If user specify own traitDef, instead of default trait,
  ...
  private SqlValidator createSqlValidator(CalciteCatalogReader catalogReader) {
    ...
    return new TrainDBCalciteSqlValidator(opTab,
        catalogReader,
        getTypeFactory(),
        sqlValidatorConfig
  ...
  private enum State {
    STATE_0_CLOSED {
      @Override void from(TrainDBPlanner planner) {
        planner.close();
      }
    },
    STATE_1_RESET {
    ...
    void from(TrainDBPlanner planner) {
